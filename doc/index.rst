...
* Make sure ``ts_ddsconfig/python`` is on the ``PYTHONPATH`` for ts_salobj.
...
* Pick an OpenSplice configuration:
    ...
    * For SAL components that listen to events from other SAL components:
      ...
      (only for events; no other SAL topics have late joiner data),
    ...
    * For SAL components that do not listen to events from other SAL components:
      you may safely use the single process mode, because no durability service is needed.
    * For local development: you may safely use the single process mode.
...
* Set environment variable ``LSST_DDS_QOS`` to point the QoS file.
  ...
  Note that ts_sal uses this environment variable, but ts_salobj calls ``lsst.ts.ddsconfig.get_qos_path`` instead.
